<template>
  <slowf-drawer title="权限管理" size="400px" :show="show" @close="$emit('close')">
    <div v-loading="loading">
      <el-tree ref="tree" :data="list" :props="{ label: 'Name', children: 'Children' }" node-key="Id" accordion :default-checked-keys="checkes" show-checkbox @check-change="onNodeCheck"></el-tree>
    </div>
  ...
  export default {
    ...
    methods: {
      ...
      async doSave () {
        this.loading = true
        try {
          let res = await apiRole.SetModules({ roleId: this.payload.Id, moduleIds: this.$refs.tree.getCheckedKeys() })
          if (res.Content && res.Type !== 200) {
            throw res.Content
          }
